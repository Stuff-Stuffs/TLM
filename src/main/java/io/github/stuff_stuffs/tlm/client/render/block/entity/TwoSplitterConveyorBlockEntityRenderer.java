...
public class TwoSplitterConveyorBlockEntityRenderer implements BlockEntityRenderer<TwoSplitterConveyorBlockEntity> {
    private final ModelPart trayModel;

    public TwoSplitterConveyorBlockEntityRenderer(final BlockEntityRendererFactory.Context context) {
        trayModel = context.getLayerModelPart(ConveyorTrayRenderer.CONVEYOR_TRAY_LAYER);
    }

    @Override
    public void render(final TwoSplitterConveyorBlockEntity entity, final float tickDelta, final MatrixStack matrices, final VertexConsumerProvider vertexConsumers, final int light, final int overlay) {
