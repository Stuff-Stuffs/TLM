...
public class LabelerBlockEntityRenderer implements BlockEntityRenderer<LabelerBlockEntity> {
    private final ModelPart trayModel;

    public LabelerBlockEntityRenderer(final BlockEntityRendererFactory.Context context) {
        trayModel = context.getLayerModelPart(ConveyorTrayRenderer.CONVEYOR_TRAY_LAYER);
    }

    @Override
    public void render(final LabelerBlockEntity entity, final float tickDelta, final MatrixStack matrices, final VertexConsumerProvider vertexConsumers, final int light, final int overlay) {
